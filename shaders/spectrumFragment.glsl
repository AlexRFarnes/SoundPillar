...
uniform sampler2D uMatCap; // matcap sent through the uniforms
...
void main() {
    ...
    // Noise
    float n3 = snoise(vec3(vPosition.xz * 5.0, uTime * 0.5)) * 0.5;

    // Waves
    float w = sin(vPosition.y * 5.0 - uTime * uWaveSpeed);
    ...
    // Invert the black with the white
    float opMask = 1.0 - vPosition.y;
    opMask *= 0.15; // Make it darker
    opMask += 0.5; // Make the gradient go further
    vec4 opMaskOut = vec4(1.0, 1.0, 1.0, opMask); // The opacity is controlled by the opMask
